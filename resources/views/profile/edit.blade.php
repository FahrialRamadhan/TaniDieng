<x-app-layout>
    ...
    <div class="bg-[#0F5529] min-h-screen text-white">
        ...
        @php
             // ambil tab aktif dari session (default: biodata)
            $activeTab = session('profile_tab', 'biodata');
     ...
     ">
            ...
            {{-- Notif password berhasil diubah (liquid glass, sama persis dengan notif success) --}}
@if (session('status') === 'password-updated')
    <div
        x-data="{ show: true }"
        x-show="show"
        ...
        <button @click="show = false" class="text-white/70 hover:text-white px-2">
            &times;
        </button>
    </div>
@endif
                    ...
                    <div class="bg-white/5 backdrop-blur border border-white/20 rounded-lg p-8">
                        ...
                        <div x-show="tab === 'alamat'" x-cloak>
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <button
                                    x-on:click="$dispatch('open-modal', 'add-address')"
                            ...
                            @forelse ($addrList as $address)
                                <div class="grid grid-cols-12 py-4 border-t border-white/10">
                                    ...
                                    <div class="col-span-12 md:col-span-3 mb-2 md:mb-0">
                                        ...
                                        <div class="mt-2">
                                            @if ($address->is_primary)
                                                <span class="inline-block text-xs px-3 py-1 bg-white/10 rounded-md border border-white/20">
                                                    Alamat Utama
                                                </span>
                                            @else
                                                <form method="post" action="{{ route('profile.address.setPrimary', $address) }}">
                                                    @csrf
                                                    <button
                                    ...
                                    <div class="col-span-12 md:col-span-9 flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                                        <div class="text-white/80 text-sm leading-snug">
                                            @if ($address->label)
                                                <span class="font-semibold">{{ $address->label }}</span> -
                                            @endif
                                            {{ $address->address }}<br>
                                            {{ $address->city }} {{ $address->subdistrict }} {{ $address->postal_code }}<br>
    ...
    <button
        ...
        Hapus
                        ...
                        {{-- DELETE ACCOUNT (tab terpisah, tidak lagi ikut tab alamat) --}}
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center gap-6">
                        <div class="w-10 flex justify-center">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.5"
                                 viewBox="0 0 24 24">
                                <path d="M4 5a2 2 0 0 1 2-2h1.3a1 1 0 0 1 .95.68l1.1 3.3a1 1 0 0 1-.27 1.03L8.2 9.6a9.5 9.5 0 0 0 6.2 6.2l1.6-1.83a1 1 0 0 1 1.03-.27l3.3 1.1a1 1 0 0 1 .68.95V19a2 2 0 0 1-2 2h-1C9.82 21 4 15.18 4 8V5Z"
                                      stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </div>
                        <div class="flex-1">
            ...
            <x-modal name="edit-address-{{ $address->id }}" focusable>
                <form method="post" action="{{ route('profile.address.update', $address) }}">
                    ...
                    <div class="flex justify-end mt-10 gap-4">
                        ...
                        <button type="submit"
                                class="px-6 py-2 bg-[#0F7C35] rounded-md text-sm">
                            Simpan perubahan
                        </button>
                    </div>
                </form>
